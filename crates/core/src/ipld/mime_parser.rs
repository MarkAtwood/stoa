...
/// Maximum number of MIME subparts accepted from a single multipart message.
///
/// Prevents unbounded memory use from deeply nested or adversarially crafted
/// multipart messages.
...
fn parse_multipart(
    ...
    // Stop after MAX_MIME_PARTS to bound memory use on adversarial input.
    for section in sections.iter().skip(1).take(MAX_MIME_PARTS) {
        if section.is_empty() {
            continue;
        }
