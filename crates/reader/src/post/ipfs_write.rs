...
mod tests {
    ...
    impl IpfsBlockStore for FailingIpfsStore {
        async fn put_raw_block(&self, _data: &[u8]) -> Result<Cid, IpfsWriteError> {
            Err(IpfsWriteError::WriteFailed("test failure".into()))
        }

        async fn get_raw_block(&self, _cid: &Cid) -> Result<Vec<u8>, IpfsWriteError> {
            Err(IpfsWriteError::NotFound("test".into()))
        }
    }
