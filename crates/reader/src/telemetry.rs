...
pub fn init_telemetry(
...
) -> (OtelGuard, Option<opentelemetry_sdk::logs::SdkLoggerProvider>) {
    let endpoint = match config.otlp_endpoint.as_deref() {
        Some(ep) => ep,
        None => {
            return (
                OtelGuard { meter_provider: None, tracer_provider: None, logger_provider: None },
                None,
            )
        }
    };
    ...
    let logger_provider = config.logs_endpoint.as_deref().and_then(|ep| {
        match build_logger_provider(ep, headers) {
            Ok(p) => Some(p),
            Err(e) => {
    ...
    (
        OtelGuard { meter_provider, tracer_provider, logger_provider },
        log_bridge_provider,
    )
}
